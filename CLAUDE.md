...
docs/decisions.md           design decision log D4-D10 + rejected alternatives
...
## Current status
**Days 1, 2 and 3 complete and verified.** Corpus built and split; embeddings, FAISS
indexes and both retrieval modes working and eyeballed; generation running end to end
with `results/summaries.csv` (450 rows) written — the Day 5 interface exists.
...
**Next: Day 4** — `app/streamlit_app.py`. Per `plan_7day.md`, this single screen *is*
the project and outranks the eval. Call `compare(query)` or `summarize(query, config=...)`
from `src.generate`; wrap index/model loading in `@st.cache_resource` or every rerun pays
the ~3 s encoder load. To demo the abstention state you must type an **off-topic** query —
`"how do I bake sourdough bread at home"` fires it; real news queries almost never will
(D9). Ship a suggested-query chip or the guardrail looks unimplemented.
...
- `docs/decisions.md` — D4 judge model, D5 reference summaries rejected (+ ROUGE-L open
  item), D6 split stays unstratified, D7 generation contract for Day 3, D8 query text =
  rewritten headline in two variants, D9 abstention threshold tau=0.35, D10 summarization
  task + relevance-tolerant prompt.
